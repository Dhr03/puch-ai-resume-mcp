...
async def resume() -> str:
    ...
    """
    try:
        # Look for resume in common formats
        resume_paths = [
            Path("resume.pdf"),
            Path("resume.docx"),
            Path("resume.txt"),
        ...
        if not resume_path:
            raise FileNotFoundError("No resume file found. Please add your resume as resume.pdf, resume.docx, resume.txt, or resume.md")
        ...
        # If it's not already markdown, convert it
        if resume_path.suffix != '.md':
            text = markdownify.markdownify(text, heading_style=markdownify.ATX)
